...
from utils_one_word import LazySupervisedDataset  # Direct import instead of * import
...
def check_data(args, dataloader, model, tokenizer):
    ...
    try:
        ...
        print(f"Batch类型: {type(first_batch)}")
        print(f"Batch键: {list(first_batch.keys())}")
        
        for key, value in first_batch.items():
            if isinstance(value, torch.Tensor):
                print(f"  {key}: tensor shape {value.shape}, dtype {value.dtype}")
                if key in ['input_ids', 'labels']:
                    print(f"    {key} 前10个值: {value[0][:10] if value.dim() > 1 else value[:10]}")
                    print(f"    {key} 长度: {value.shape[-1]}")
            else:
...
def process_sample_attention(model, tokenizer, sample_input_ids, sample_image, sample_id, vision_token_start, vision_token_end, output_path):
...
def collect_attention(args, dataloader, model, tokenizer):
    ...
    if all_attention_data:
        ...
        print(f"所有样本的注意力数据已保存到: {combined_data_path}")
        print(f"总共处理了 {len(all_attention_data)} 个样本")
    else:
        print("没有成功处理任何样本，没有数据可保存")
...
def main():
    ...
    # Model parameters
    parser.add_argument("--model_path", type=str, default="/data2/jkx/LLaVA/checkpoints/llava-v1.5-7b-lora", help="Path to the model")
    parser.add_argument("--output_path", type=str, default="/data2/jkx/LLaVA1/output/attention_info", help="Path to save outputs")
    
    # Data parameters
    parser.add_argument("--data_path", type=str, default="/data2/jkx/LLaVA/output/chunk_json/chunk_1.json", help="Path to the json file")
