...
from obtain_attention_utils_one_word import LazySupervisedDataset  # Direct import instead of * import
...
def process_sample_attention(model, tokenizer, sample_input_ids, sample_image, sample_image_aux, sample_id, vision_token_start, vision_token_end, output_path):
    ...
    with torch.inference_mode():
        try:
            ...
            # 堆叠所有token的注意力图为单个张量: [tokens, vision_tokens]
            ...
            attention_data = {
                'sample_id': sample_id,
                'token_ids': token_ids_tensor,  # 所有生成的token IDs
                'attention_maps': attention_tensor.detach().cpu(),  # 注意力图: [tokens, vision_tokens]
                'relative_attention_maps': relative_attention_tensor.detach().cpu()  # 相对注意力图: [tokens, vision_tokens]
            }
