...
def aggregate_llm_attention(attn):
    ...
    return torch.stack(avged).mean(dim=0)#平均的是同一层里不同样本（batch）的 vec，和层数无关
...
# def show_mask_on_image(img, mask, alpha=0.5, blur=True, cmap="jet"):
#     """
#     Overlay attention mask on image.

#     Args:
#         img (np.ndarray): shape [H, W, 3], RGB image in uint8
#         mask (np.ndarray): shape [H, W], attention weights normalized to [0,1]
#         alpha (float): blending factor
#         blur (bool): whether to apply gaussian blur
#         cmap (str): matplotlib colormap name ("jet", "viridis", etc.)
#     Returns:
#         overlayed image, raw heatmap
#     """
#     if mask.max() > 1.0:
#         mask = mask / 255.0
#     if blur:
...
#     heatmap = cm.get_cmap(cmap)(mask)[:, :, :3]  # [H, W, 3], RGB
#     heatmap = np.uint8(heatmap * 255)

#     if img.dtype != np.uint8:
#         img = np.uint8(img * 255) if img.max() <= 1.0 else img
