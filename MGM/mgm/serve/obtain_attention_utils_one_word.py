...
def extract_gpt_responses(
    ...
    """
    Extract messages from conversations that lead to GPT responses.
    Only includes the human messages, not the assistant responses.
    
    Args:
        sources: List of conversations where each message has 'from' and 'value' fields
        tokenizer: Tokenizer to use for encoding
        image_token_index: Index of the image token if images are present
        model_config: Configuration of the model for image processing
        
    Returns:
        input_ids: Tensor containing the tokenized human messages
    """
    # Extract all human messages that have an assistant response
    human_messages = []
    
    for source in sources:
        for i in range(len(source)):
            if source[i].get("from", "").lower() in ["gpt", "assistant"] and i > 0:
                # Check if the previous message is from human
                if source[i-1].get("from", "").lower() in ["human", "user"]:
                    # Only collect the human message
                    human_messages.append(source[i-1]["value"])
    
    # Process the messages similar to inference1 copy.py
    all_input_ids = []
    
    for human_msg in human_messages:
        # Check if message contains image tokens


        if DEFAULT_IMAGE_TOKEN not in human_msg:
            # Add image token to the beginning if not present
            human_msg = DEFAULT_IMAGE_TOKEN + "\n" + human_msg
        
        conv_mode = "vicuna_v1"    
        
        human_msg = human_msg + " Please answer the question using a single word or phrase."
        # Create conversation using the template
        conv = conversation_lib.conv_templates[conv_mode].copy()
        conv.append_message(conv.roles[0], human_msg)  # Only add human message
    ...
    # Combine all input_ids into a single tensor if there are multiple, length different, cannot concatenate
    # if all_input_ids:
    ...
    # else:
    #     return torch.empty(0, dtype=torch.long)  # Return empty tensor if no human messages

    return dict(input_ids=all_input_ids[0], labels=None)
...
class LazySupervisedDataset(Dataset):
    ...
    def __getitem__(self, i) -> Dict[str, Union[torch.Tensor, str, int]]:
        ...
        if 'image' in sources[0]:
            ...
            if hasattr(self.data_args.model_config, 'image_size_aux'):
                if not hasattr(processor, 'image_size_raw'):
                    processor.image_size_raw = processor.crop_size.copy()
                processor.crop_size['height'] = self.data_args.model_config.image_size_aux
                processor.crop_size['width'] = self.data_args.model_config.image_size_aux
                processor.size['shortest_edge'] = self.data_args.model_config.image_size_aux
            ...
            if self.data_args.image_aspect_ratio == 'pad':
                ...
                image = processor.preprocess(image, return_tensors='pt')['pixel_values']
            raw_shape = [processor.image_size_raw['height'] * 1,
                processor.image_size_raw['width'] * 1]
            image_tensor_aux = image 
            image = torch.nn.functional.interpolate(image,
                                                        size=raw_shape,
                                                        mode='bilinear',
                                                        align_corners=False)[0]
            image_tensor_aux = image_tensor_aux[0]
        ...
        data_dict = extract_gpt_responses(sources, self.tokenizer)
        if isinstance(i, int):
            data_dict = dict(input_ids=data_dict["input_ids"][0])
        # image exist in the data
        if 'image' in self.list_data_dict[i]:
            data_dict['image'] = image
            data_dict['image_aux'] = image_tensor_aux
        elif self.data_args.is_multimodal:
            # image does not exist in the data, but the model is multimodal
            crop_size = self.data_args.image_processor.crop_size
            data_dict['image'] = torch.zeros(3, processor.image_size_raw['height'], processor.image_size_raw['width'])
            data_dict['image_aux'] = torch.zeros(3, crop_size['height'], crop_size['width'])
        
        # Add unique_id to the returned data if it exists
        if 'unique_id' in self.list_data_dict[i]:
            data_dict['unique_id'] = self.list_data_dict[i]['unique_id']
